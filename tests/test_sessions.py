...
@pytest.mark.asyncio
async def test_cookie_read_invalid_signed_cookie(cookie):
...
def test_middleware(app, test_client):
    ...
    backend = InMemoryBackend({"sid": {"username": "root"}})
    app.middleware.use(SessionMiddleware, secret_key="key", backend=backend)
    app.routes.get("/", view)
    response = test_client.get("/", cookies={"session": "sid"})
    assert response.json() == {"username": "root"}


def test_middleware_https_only(app, test_client):
    def view(request: Request):
...
def test_middleware_clears_session(app, test_client):
    def view(request: Request):
    ...
    app.routes.get("/", view)
    response = test_client.get("/")
    assert "null" in response.headers["Set-Cookie"]
